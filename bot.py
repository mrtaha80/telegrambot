...
def process_game_data(raw_text, image_bytes=None, fallback_id="0", channel_id=1):
    try:
        ...
        for line in lines:
            line_str = line.strip()
            if not line_str or any(w in line_str.lower() for w in ['event', 'ایونت', 'time', 'scenario', 'سناریو', 'date', 'تاریخ', 'winner', 'برنده', 'win', 'players', 'بازیکنان']):
                continue
        ...
        if len(temp_players) < 5:
            ...
            for line in lines:
                line_str = line.strip()
                if any(w in line_str.lower() for w in ['event', 'ایونت', 'time', 'scenario', 'سناریو', 'date', 'تاریخ', 'win', 'برنده', 'players', 'بازیکنان']):
                    continue
...
async def link_profile_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if player_name in HIDDEN_PLAYERS:
...
async def show_player_card(update: Update, query_name: str, ch_id: int, ch_name: str):
    ...
    if query in HIDDEN_PLAYERS:
        await update.message.reply_text(f"❌ اطلاعات آماری برای این بازیکن در دسترس نیست.", reply_markup=get_main_keyboard())
        return
